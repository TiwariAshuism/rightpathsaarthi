...
const Pricing: React.FC = () => {
	...
	const { ref: cardsRevealRef, isVisible: cardsVisible } =
	...
	return (
		<div className="min-h-screen bg-white dark:bg-[#101922] font-display flex flex-col text-[#111418] dark:text-white transition-colors duration-1000">
			...
			<main className="flex-1">
				...
				<section
					ref={heroRevealRef}
					className="relative pt-24 pb-20 px-4 md:px-10 lg:px-40 text-center overflow-hidden"
				>
					{/* Mesh Gradient Background */}
					<div className="absolute inset-0 mesh-gradient opacity-20 dark:opacity-10 pointer-events-none"></div>

					<div
						className={`max-w-4xl mx-auto relative z-10 transition-all duration-1000 ${heroVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
					>
					...
					<div className="max-w-[1280px] mx-auto grid grid-cols-1 md:grid-cols-3 gap-10">
						{pricing.plans.map((plan, index) => (
							<div
								key={index}
								ref={tiltRefs[index]}
								className={`relative flex flex-col p-10 rounded-[2.5rem] transition-all duration-1000 border overflow-hidden ${
									plan.highlight
							...
							>
								...
								<div className="relative z-10 mt-auto">
									<Link
										to="/contact"
										className={`w-full py-5 px-8 rounded-2xl text-center font-black text-lg block transition-all active:scale-95 ${
											plan.highlight
				...
				<section className="py-32 bg-slate-50/50 dark:bg-slate-900/20 border-t border-slate-100 dark:border-slate-800 px-4 md:px-10 lg:px-40 transition-colors duration-1000">
					<div className="max-w-5xl mx-auto">
						<div className="text-center mb-20">
							<span className="text-primary font-black text-sm tracking-widest uppercase mb-4 block">
								Got Questions?
							</span>
