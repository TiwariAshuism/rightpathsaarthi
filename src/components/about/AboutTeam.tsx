...
export const AboutTeam = (): FunctionComponent => {
	...
	const otherPlatformsRef = useMouseTilt<HTMLDivElement>({
		max: 5,
		perspective: 1000,
		scale: 1.02,
	});
	const rightPathSarthi = useMouseTilt<HTMLDivElement>({
		max: 5,
		perspective: 1000,
		scale: 1.02,
	});
	...
	return (
		<section
			ref={revealRef}
			className="bg-white dark:bg-[#101922] py-24 px-4 md:px-10 lg:px-40 relative overflow-hidden transition-colors duration-1000"
		>
			{/* Background blob */}
			...
			<div className="max-w-[1200px] mx-auto relative z-10">
				<div
					className={`flex flex-col gap-4 mb-16 text-center transition-all duration-1000 ${
						isVisible
					...
					>
						...
						<div className="flex flex-col gap-5">
							{team.otherPlatforms.features.map((feature, index) => (
								...
								>
									<span className="text-red-500 dark:text-red-400 text-2xl font-bold mt-0.5 flex-shrink-0">
										×
									</span>
							...
							<div className="flex flex-col gap-5">
								{team.rightPathSarthi.features.map((feature, index) => (
									<div
										...
										style={{ transitionDelay: `${index * 0.1}s` }}
									>
										<span className="text-primary dark:text-primary text-2xl font-bold mt-0.5 flex-shrink-0">
											✓
										</span>
