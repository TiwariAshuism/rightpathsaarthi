...
const ServicesHero: React.FC<ServicesHeroProps> = ({
	...
	return (
		<section
			ref={ref}
			className="relative bg-white dark:bg-[#101922] py-20 lg:py-32 px-6 lg:px-40 transition-colors duration-1000 overflow-hidden"
		>
			{/* Mesh Gradient Background */}
			<div className="absolute inset-0 mesh-gradient opacity-20 dark:opacity-10 pointer-events-none"></div>

			<div
				className={`max-w-[1200px] mx-auto flex flex-col gap-10 items-center text-center relative z-10 transition-all duration-1000 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
			>
