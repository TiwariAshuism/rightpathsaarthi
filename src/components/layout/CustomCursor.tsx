...
export const CustomCursor: React.FC = () => {
	...
	return (
		<>
			<div
				ref={cursorDotRef}
				className={`fixed top-0 left-0 size-2 bg-primary rounded-full z-[100] pointer-events-none -translate-x-1/2 -translate-y-1/2 transition-opacity duration-300 ${
					isHidden ? "opacity-0" : "opacity-100"
				}`}
			/>
			<div
				ref={cursorOutlineRef}
				className={`fixed top-0 left-0 size-10 border-2 border-primary/30 rounded-full z-[99] pointer-events-none -translate-x-1/2 -translate-y-1/2 transition-all duration-300 ease-out ${
					isHidden ? "opacity-0" : "opacity-100"
